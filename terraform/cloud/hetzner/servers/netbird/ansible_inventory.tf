    ...
    yamlencode({
      all = {
        children = {
          public_vps = {
            hosts = {
              for vm_name in sort(keys(local.ansible_inventory_vms)) : local.ansible_inventory_vms[vm_name].name => {
                ansible_host = local.ansible_inventory_vms[vm_name].ipv4_address
