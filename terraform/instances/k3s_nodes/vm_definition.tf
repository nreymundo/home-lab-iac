    ...
    for node_index, node in local.k3s.nodes : merge(
      {
        ...
        proxmox_tags = distinct(concat(
          ["k3s-node"],
          try(node.proxmox_tags, null) == null ? [] : node.proxmox_tags,
          try(node.pci_devices, null) == null ? [] : (length(node.pci_devices) > 0 ? ["gpu"] : [])
        ))
        pci_devices = try(node.pci_devices, null) == null ? [] : node.pci_devices
      },
