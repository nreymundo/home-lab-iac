source "proxmox-iso" "ubuntu-docker" {
  ...
  vm_name              = "${local.default_name}-docker"
  template_description = "${local.default_description} with Docker"
...
build {
  ...
  provisioner "ansible" {
    playbook_file = "${path.root}/../../../ansible/playbooks/ubuntu/docker.yml"
